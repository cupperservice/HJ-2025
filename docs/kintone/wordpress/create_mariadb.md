...
### MariaDB に接続する
`mysql -uroot -p` を実行すると `Enter password:` と表示されるので、上記で設定したパスワードを入力します。

### WordPress 用データベースを作成する
データベースを作成します。
...
### ユーザーに権限を付与する
作成したユーザーにデータベース (wordpress-db) へのアクセス権限を付与します。
...
### MariaDB に外部から接続できるようにする
MariaDB の設定ファイルを編集して、外部から接続できるようににします。
